...
```bash
WS-POC$ sam local invoke SQSPayloadLogger --event events/event-sns.json
```
...
Resources:
  ...
  SQSPayloadLogger:
    ...
    Properties:
      CodeUri: ./src/WS-POC/
      Handler: WS-POC::WS-POC.Function::FunctionHandler
...
```bash
WS-POC$ sam logs -n SNSPayloadLogger --stack-name sam-app --tail
```
...
```bash
WS-POC$ dotnet test test/WS-POC.Tests/WS-POC.Tests.csproj 
```
...
```bash
WS-POC$ aws cloudformation delete-stack --stack-name sam-app
```
